...
- (void)onDeviceReady:(NSMutableArray *)arguments withDict:(NSMutableDictionary*)options {
  ...
  if(pushHandler.startPushData) {
    NSString *jsStatement = [NSString stringWithFormat:@"window.plugins.pushNotification.notificationCallback(%@, %@);", pushHandler.startPushData, [NSNumber numberWithBool: YES]];
...
- (void) onPushAccepted:(PushNotificationManager *)manager withNotification:(NSDictionary *)pushNotification onStart:(BOOL)onStart{
  ...
  NSString *jsStatement = [NSString stringWithFormat:@"window.plugins.pushNotification.notificationCallback(%@, %@);", jsonString, [NSNumber numberWithBool: onStart]];
